...
<html lang="en">
   <head>
      ...
      <style>
        .draggableRect {width: 50px; height: 30px;background: #CCC; overflow: visible;position: absolute !important }
         #safety-line{position: relative;background-image:url('');
         background-size:cover;
         background-position: center; margin: auto;}
         .bring-forward{z-index: 0;}
         .move-backwrd{z-index: 1;}
       /*  #bleed-line{
   ...
   // Code for generating draggable Text
    $(document).ready(function () {
        ...
        $("#create_text").click(function () {
          $(".canvasmodule").append($('<div class="draggableText" data-unique="draggableText'+designertext+'"  id="draggableText'+ designertext + '"><span class="posX text-coordinate"></span><span class="posY text-coordinate"></span><span class="poswidth text-coordinate"></span><span class="posheight text-coordinate"></span><span class="posrotate text-coordinate"></span><div class="textShow">Click here To edit Text</div><div class="ui-resizable-handle ui-resizable-nw" id="nwgrip"></div><div class="ui-resizable-handle ui-resizable-ne" id="negrip"></div><div class="ui-resizable-handle ui-resizable-sw" id="swgrip"></div><div class="ui-resizable-handle ui-resizable-se" id="segrip"></div><div class="ui-resizable-handle ui-resizable-n" id="ngrip"></div><div class="ui-resizable-handle ui-resizable-e" id="egrip"></div><div class="ui-resizable-handle ui-resizable-s" id="sgrip"></div><div class="ui-resizable-handle ui-resizable-w" id="wgrip"></div></div>').draggable({containment: '#bleed-line'}).resizable({containment: "#bleed-line",handles: {
                'ne': '#negrip',
                'se': '#segrip',
                'sw': '#swgrip',
                'nw': '#nwgrip',
    ...
    $(document).on("mousemove", ".draggableText", function () {
      $('.draggableText').find('.ui-resizable-handle').hide();
      $('.draggableText').css('border','');
      $(this).css('border','1px dashed #848383');
       $(this).find('.ui-resizable-handle').show();
      // Code for finding the coordinates of text box start
      var parentcoord = $('.canvasmodule').position();
      var grntparntL = $('.canvasmodule').position().left;
    ...
    // code to check if textbox area already exist
      if(uniquedataClass==0){
        ...
        $(".slider").slider({
          ...
          slide: function(event, ui) {
            ...
            $('#'+textBoxid).find(".posrotate").text(ui.value);
      ...
      $('.optionPanel.'+uniqueclass).find('.postxtheight').val(height);
       ...
       if(posLayoutright<0 || posLayoutleft<0){
        // $('.optionPanel').append('error');
      }


      $('.movebckbtn').click(function(){
         $('#'+textBoxid).addClass('move-backwrd');
      });
    ...
    $( function() {
      ...
      $("#create_block").click(function () {
        $(".canvasmodule").append($('<div class="draggableRect" id="draggableRect'+boXno+'"><ul class="list-unstyled" style="display:none;"><li class="posX" ></li><li class="posY" ></li><li class="poswidth" ></li><li class="posheight" ></li><li class="posrotate" ></li></ul><div class="ui-resizable-handle ui-resizable-nw" id="nwgrip"></div><div class="ui-resizable-handle ui-resizable-ne" id="negrip"></div><div class="ui-resizable-handle ui-resizable-sw" id="swgrip"></div><div class="ui-resizable-handle ui-resizable-se" id="segrip"></div><div class="ui-resizable-handle ui-resizable-n" id="ngrip"></div><div class="ui-resizable-handle ui-resizable-e" id="egrip"></div><div class="ui-resizable-handle ui-resizable-s" id="sgrip"></div><div class="ui-resizable-handle ui-resizable-w" id="wgrip"></div></div>').draggable({containment: ".bleed-line"}).resizable({containment: "#bleed-line",
            handles: {
                'ne': '#negrip',
                'se': '#segrip',
                'sw': '#swgrip',
    ...
    $(document).on('mousemove','.draggableRect',function(e){   
        var shapeId= this.id;
        $(this).find('.ui-resizable-handle').show();
        $(".draggableRect").css("border",'1px solid #a5d048');
        var parentcoord = $('.canvasmodule').position();
        var grntparntL = $('.canvasmodule').position().left;
        var coord = $(this).position();
        var posLayoutleft = (coord.left - grntparntL);
        ...
        $(this).find(".posheight").text(height);
         ...
         if(posLayoutleft<0 || posLayoutright<0){
          $('.shapeboxcord').append('Error ');
         }
...
// Code to upload ultiple image 
    ...
    if (window.File && window.FileList && window.FileReader) {
      ...
      $("#stickers").on("change", function(e) {
        ...
        fileReader.onload = (function(e) {
          var file = e.target;
          $(".canvasmodule").append($("<div id=\"pip" +imgThumb+ "\" data-transrotat=\"imageThumb"+imgThumb+"\" class=\"pip\">" +
                  "<img id=\"imageThumb" +imgThumb+ "\" posx='' posrot='' posy='' posheight='' poswidth='' class=\"imageThumb\" src=\"" + e.target.result + "\" title=\"" + file.name + "\"/><span class=\"posX\"></span></div>").draggable({  containment: "#bleed-line"}));
  ...
  $(document).on("mouseover",".imageThumb",function(){
    $(this).resizable({containment: "#bleed-line",aspectRatio: true}).mousemove(function(){
      var parentcoord = $('.canvasmodule').position();
      var $img = $(this);
       ...
       $('.customtextPanel').html($('<div class="imgtagcord">X:<input type="text" value="'+posLayoutleft+'"  class="posXimg" readonly>Y:<input type="text" class="posYimg" value="'+posLayoutright+'" readonly>W:<input type="text" class="poswidthimg" value="'+width+'">H:<input type="text" class="posheightimg" value="'+height+'" >R:<input type="text" class="rotateimg" value="" ><div>ROTATION SLIDER:</div><div id="imgslider" data-imgrotate="'+imGid+'" class="slider"></div><div class="removeimg" data-textparent="'+imGid+'" >Remove</div></div>'));
         ...
         $(".slider").slider({
              ...
              slide: function(event, ui) {
                ...
                $("#"+outerDiv).css('transform', 'rotate(' + ui.value + 'deg)')
...
var getCanvas; // global variable
      ...
      for(var i=1;i<=uniqueclass;i++){
        ...
        var boXrotate = $("#draggableRect"+i).find(".posrotate").text();
        item = {}
        item ["id"] = boXid;
      ...
      for(j=1;j<=imgUniqueClass;j++){
        ...
        var imgrotate = $("#imageThumb"+j).attr('posrot');
      ...
      for(k=1;k<=txtUnique;k++){
        ...
        var txtrotate = $("#draggableText"+k).find('.posrotate').text();
